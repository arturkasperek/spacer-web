...
export type NpcVobBvhCollisionResult = { x: number; z: number; collided: boolean };

export function applyNpcVobBVHCollisionXZ(
  ctx: NpcWorldCollisionContext,
  npcGroup: THREE.Object3D,
  desiredX: number,
  desiredZ: number,
  vobMeshes: THREE.Mesh[],
  config: NpcWorldCollisionConfig
): NpcVobBvhCollisionResult {
  const fromX = npcGroup.position.x;
  const fromZ = npcGroup.position.z;
  const dx = desiredX - fromX;
  const dz = desiredZ - fromZ;
  const dist = Math.hypot(dx, dz);
  if (!Number.isFinite(dist) || dist < 1e-6 || !vobMeshes.length) {
    return { x: desiredX, z: desiredZ, collided: false };
  }

  const radius = Math.max(1e-6, config.radius);
  const substeps = Math.min(10, Math.max(1, Math.ceil(dist / radius)));
  const stepDx = dx / substeps;
  const stepDz = dz / substeps;

  // Use current Y (already ground-snapped / world-collision resolved) as collider base.
  const baseY = npcGroup.position.y;

  // Treat VOB triangles as obstacles unless they are perfectly floor-like.
  // This prevents climbing sloped props and keeps behavior stable.
  const minWalkableNy = 0.99;
  ...
  for (let s = 0; s < substeps; s++) {
    ...
    for (const mesh of vobMeshes) {
      ...
      ctx.tmpOrigin.set(x, baseY, z).applyMatrix4(ctx.invMatrixWorld);
      const capsule = buildCapsuleLocal(ctx, ctx.tmpOrigin, config);
      const res = resolveCapsuleIntersections(ctx, bvh, capsule, {
        maxIterations: 6,
        planarOnly: true,
        minWalkableNy,
      ...
      // Convert capsule-local back to world-space NPC origin (feet).
      ...
      x = originLocalAfter.x;
      z = originLocalAfter.z;
      collided = true;
    }
  }
