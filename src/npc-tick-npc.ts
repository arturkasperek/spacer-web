    ...
    for (const npcGroup of loadedNpcsRef.current.values()) {
      ...
      const tickNpcUiStage = () => {
        ...
        if (showJumpDebugRange && isHeroForDebug) {
          ...
          const root = (npcGroup.userData as any)?._jumpDebugLabelRoot as
            | THREE.Object3D
            | undefined;
        ...
        if (
          ...
          if (fill?.position)
            fill.position.x = -healthBar.width / 2 + (healthBar.width * ratio) / 2;
              ...
              if (!ud._kccJumpAnimActive) {
                ...
                if (isJumpUpHigh && highHangPlayed && !highStandPlayed) {
                  if (
                    ...
                    const standRef = resolveNpcAnimationRef(
                      npcData.instanceIndex,
                      "T_HANG_2_STAND",
                    );
                    const highBlendMs = 80;
              ...
              if (ud._kccJumpAnimActive) {
                ...
                if (moving) {
                  ...
                  instance.setAnimation(nextRef.animationName, {
                    ...
                    resetTime: true,
                    blendInMs:
                      locomotionMode === "run" && jumpStartWasRun ? 200 : nextRef.blendInMs,
                    blendOutMs:
      ...
      const tickNpcDebugStage = () => {
        if (shouldLogMotion) {
          ...
          const floorTargetY = (npcGroup.userData as any)?._fallDbg?.floorTargetY as
            ...
            | undefined;
          const lastWarnNext =
            isFallingNow && floorTargetY == null && nowMs - lastWarnAtMs > 500
              ? nowMs
              : lastWarnAtMs;
