...
function buildVOBTree(world: World): TreeNode[] {
  ...
  const buildNode = (vob: Vob, depth: number, path: string): TreeNode => {
    ...
    const visualTypeName =
      visualType >= 0 && visualType < typeNames.length
...
function buildWaypointsTree(world: World): TreeNode[] {
  ...
  try {
    ...
    if (
      !waypointsVector ||
      typeof waypointsVector.size !== "function" ||
      typeof waypointsVector.get !== "function"
    ) {
      return [];
    }
...
export function VOBTree({
  ...
  const filterTree = (nodes: TreeNode[], searchTerm: string): TreeNode[] => {
    ...
    const filterNode = (node: TreeNode): TreeNode | null => {
      const matches =
        ...
        false ||
        node.vobName?.toLowerCase().includes(search) ||
        false;
  ...
  const filteredTree = useMemo(() => filterTree(rootTree, searchTerm), [rootTree, searchTerm]);
  ...
  const findWaypointInTree = useCallback(
    (waypointName: string, nodes: TreeNode[]): TreeNode | null => {
      for (const node of nodes) {
  ...
  const findPathToNode = useCallback(
    (targetNode: TreeNode, nodes: TreeNode[], path: TreeNode[] = []): TreeNode[] | null => {
      for (const node of nodes) {
  ...
  const RowComponent = useCallback(
    ({ index, style }: { index: number; style: CSSProperties }) => {
      ...
      const isSelectedWaypoint =
        ...
        node.kind === "waypoint" &&
        node.waypoint?.name === selectedWaypoint.name;
      ...
      >
        {flattenedItems.length > 0 ? (
          <List<Record<string, never>>
            ...
            rowComponent={
              RowComponent as (
                props: { index: number; style: CSSProperties } & Record<string, never>,
              ) => React.ReactElement
            }
