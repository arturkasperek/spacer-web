...
function VOBRenderer({
  ...
  const hotVobPoolRef = useRef(
    new HotObjectPool<THREE.Object3D>(HOT_VOB_CACHE_MAX_BYTES, (obj) => disposeObject3D(obj)),
  );
  ...
  const cacheHotVob = (obj: THREE.Object3D): void => {
    ...
    hotVobPoolRef.current.put(visualKey, obj, approxBytes);
  };
  ...
  const restoreHotVob = (visualName: string, vobId: string, vob: Vob): THREE.Object3D | null => {
    ...
    const cached = hotVobPoolRef.current.take(visualKey);
    if (!cached?.value) return null;
    const obj = cached.value;
  ...
  const updateVOBStreaming = () => {
    ...
    if (onVobStats) {
      ...
      const hotStats = hotVobPoolRef.current.getStats();
      onVobStats({
        loaded: loadedCount,
        total: totalCount,
        ...
        hotObjectCacheCount: hotStats.count,
        hotObjectCacheBytes: hotStats.bytes,
        hotObjectCacheLimitBytes: hotStats.limitBytes,
      });
    }
