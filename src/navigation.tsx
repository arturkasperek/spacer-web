...
export function NavigationBox({ onCameraChange }: NavigationBoxProps) {
  ...
  return (
    <group scale={0.5}>
      ...
      {/* Back face */}
      <mesh position={[0, 0, -0.5]} rotation={[0, Math.PI, 0]} onClick={() => handleFaceClick('back')}>
        <planeGeometry args={[1, 1]} />
        <meshBasicMaterial color="#4ecdc4" />
      </mesh>
      {/* Left face */}
      <mesh position={[-0.5, 0, 0]} rotation={[0, -Math.PI / 2, 0]} onClick={() => handleFaceClick('left')}>
        <planeGeometry args={[1, 1]} />
        <meshBasicMaterial color="#45b7d1" />
      </mesh>
      {/* Right face */}
      <mesh position={[0.5, 0, 0]} rotation={[0, Math.PI / 2, 0]} onClick={() => handleFaceClick('right')}>
        <planeGeometry args={[1, 1]} />
        <meshBasicMaterial color="#f9ca24" />
      </mesh>
      {/* Top face */}
      <mesh position={[0, 0.5, 0]} rotation={[-Math.PI / 2, 0, 0]} onClick={() => handleFaceClick('top')}>
        <planeGeometry args={[1, 1]} />
        <meshBasicMaterial color="#6c5ce7" />
      </mesh>
      {/* Bottom face */}
      <mesh position={[0, -0.5, 0]} rotation={[Math.PI / 2, 0, 0]} onClick={() => handleFaceClick('bottom')}>
        <planeGeometry args={[1, 1]} />
